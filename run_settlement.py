...
_frozen = getattr(sys, "frozen", False)  # PyInstaller exe 여부
if not _frozen:
    ...
    # xlrd: .xls(구형) 파일 지원용 — 미설치 시 .xls 사용 시점에 안내 메시지 표시
    # (자동 설치 제거: 사내망 nexus 타임아웃으로 GUI 시작 지연 발생)
...
_TAX_EXEMPT_KW = [
    ...
    # 근거: 부가가치세법 시행규칙 제24조 [별표 1]
    # ※ 기름·조미료 첨가 가공품은 과세 → _TAX_EXCLUDE_KW 참조
    "생선", "수산물", "해물", "어패류",
    "연어", "고등어", "갈치", "조기", "광어", "우럭",
    # ※ "참치" 단독 제외 — 상업 구매 맥락에서 거의 통조림(과세) → _TAX_EXCLUDE_KW 참조
    #   생참치·참치회는 _KNOWN_EXEMPT_NAMES에서 별도 관리
    "방어", "삼치", "가자미", "명태", "대구", "꽁치",
    "꽃게", "대게", "킹크랩",
    ...
    "자몽", "망고", "파인애플", "키위",
    # ※ "오렌지","레몬","바나나" 제외: 색상·향·음료명으로 오탐 빈번 → _KNOWN_EXEMPT_NAMES에서 복합어로 관리
    "블루베리", "체리",
    # ※ "배"(pear)는 오탐 위험(배송·배달 등)으로 _KNOWN_EXEMPT_NAMES에서 복합어로만 관리
...
_KNOWN_EXEMPT_NAMES = [
    ...
    "마른김", "건조김", "재래김", "돌김", "도시락김",
    "꿀", "허니",
    "허니스틱",        # '허니' 뒤에 '스틱' → 경계 실패 → 명시
    "멸균우유",        # '우유' 앞에 '균' → 경계 실패 → 명시 (면세)
    "락토프리",        # 유당분해 우유 = 면세
    "과일세트",        # '과일' 뒤에 '세' → 경계 실패 → 명시
    "배세트", "배선물", "배선물세트",  # '배'(배) 단독 키워드 제외로 복합어 명시 등록
    "견과류",          # '견과' 뒤에 '류' → 경계 실패 → 명시

    # 육류 복합어 — '소고기·한우' 경계 실패 케이스 보완
    # ※ 가공육(햄·소시지 등)은 _TAX_EXCLUDE_KW에서 차단
    "소고기세트", "소고기선물세트",
    "한우세트", "한우갈비", "한우등심", "한우선물세트",
    "삼겹살", "목살",          # 돼지고기 부위 — 단독 '돼지고기'로 탐지 안 되는 부위명
    "닭가슴살", "닭다리", "닭볶음용",
    "오리로스", "오리훈제",    # 훈제오리는 가공품(과세)이므로 담당자 확인 필요

    # 수산물 복합어
    "굴비", "굴비세트",
    "황태", "황태채", "북어채",   # 건조 명태 — 면세 (시행규칙 별표1)
    # 생참치·참치회 — 신선·냉동 상태 면세 (통조림과 구분)
    "생참치", "참치회", "참치횟감", "참치회세트",
    "가자미식혜",               # 식혜는 가공품이나 담당자 확인 유도

    # ── 제4호: 여성 생리처리 위생용품 복합어 ────────────────────────
    "생리용품", "생리위생용품",

    # ── 도서류 — '도서' 키워드 경계 실패 보완 ────────────────────────
    "도서구매",        # '도서' 뒤에 '구' → 경계 실패 → 명시
    "해외도서",
    "성경", "성경전서",  # 종교 출판물 — '전서' 붙으면 경계 실패 → 명시
    "이북", "e-book", "ebook",   # 전자책 영문·혼용 표기

    # ── 용역·서비스 — 과세/면세 경계 품목 (비과세 오기입 탐지) ──────────────
    # 교육·연수: 교육훈련 목적 시 면세(부가세법 제26조 제1항 제6호), 관광·포상 시 과세
    # ※ '비과세'로 잘못 태그된 경우를 면세 검토 필요로 플래그
    "연수", "기업연수", "국내연수", "해외연수", "임직원연수",
    # 온라인 교육 콘텐츠: 인가 교육기관 제공 시 면세, 일반 SW 임차·비교육 콘텐츠 시 과세
    "온라인교육", "온라인 교육", "이러닝", "e-learning",
]
...
_TAX_EXCLUDE_KW = [
    ...
    "도서산간", "제주도", "산간", "도서지역",
    "수수료", "설치비", "철거비", "인건비",
    # ── 가공식품·공산품 (명백 과세 — 오탐 방지) ─────────────────────
    "과자", "세제",
    # 가공유 (과세) — 향료·당분·영양강화 성분 첨가 시 가공식품 분류
    # ※ 경계 매칭으로 이미 차단되나 '딸기 우유'처럼 공백 표기 시 통과 가능 → 명시 차단
    "딸기우유", "딸기 우유",
    "초코우유", "초코 우유", "초콜릿우유",
    "바나나우유", "바나나 우유",
    "영양강화우유", "단백질우유",
    "가공유",
    # 가공육 (과세) — 염지·훈연·가열 등 2차 가공으로 원생산물 성질이 변함
    # 근거: 부가가치세법 시행령 제34조 미가공식료품 목록 미포함
    "햄", "소시지", "베이컨", "스팸", "핫도그", "살라미",
    "육가공", "가공육",
    # 소금 가공품 (과세) — 면세 소금 = 천일염·재제소금에 한정
    # 근거: 국세청 사전-2015-법령해석부가-22498 (2015.3.17.)
    #       부가세법 시행령 제34조 제1항 제13호
    "암염", "핑크소금", "히말라야소금", "히말라야 소금",   # ① 암염
    "죽염",                                                 # ② 죽염 (대나무 소성 공정)
    "구운소금",                                             # ③ 구운소금 (고온 가열)
    # 식용유·유지류 (과세) — 압착·정제 공정으로 원생산물 성질이 변하는 가공품
    # ※ 선물세트 내 식용유 포함 시 규격 필드 검색(_EXEMPT_SPEC_TRIGGER_KW)으로 자동 차단
    "식용유", "식용유세트",
    # 조미 해조류 (과세) — 기름·소금·조미료 첨가 또는 굽거나 가공한 제품
    # 근거: 시행규칙 제24조 [별표 1] — 면세 김 = 신선·냉장·냉동·염장·염수장·건조에 한정
    ...
    "볶음견과", "구운견과", "가공견과",
    # 통조림류 (과세) — 살균·밀봉 가공
    # ※ 참치통조림·꽁치통조림 등 수산물 통조림 포함
    "통조림", "캔참치", "참치캔",
    # 참치 브랜드·가공품 (과세) — 상업 구매 맥락에서 '참치' 단독은 거의 통조림
    "동원참치", "동원 참치",
    "사조참치", "사조 참치",
    "오뚜기참치", "오뚜기 참치",
    "참치통조림", "참치캔세트", "참치선물세트",
    ...
    "에이드", "스무디",
    "과일청",         # 설탕 첨가 → 과세
    # 화장품·개인위생용품 (과세) — 향기·성분명이 식품 키워드와 겹치는 오탐 방지
    # 예: '허니 캐모마일 바디로션' → '허니' 탐지 차단 (허니 = 화장품 향, 꿀 아님)
    "바디로션", "바디워시", "보디로션", "보디워시",
    "핸드크림", "핸드로션",
    "샴푸", "린스", "컨디셔너",
    "화장품",
    # 대리점·지점 거래 (과세) — 상호명에 식품명 포함 시 오탐 방지
    # 예: '대리점 가구_레몬 월곡점' → '레몬' 탐지 차단 (레몬 = 가구점 상호명)
    "대리점",
    # ── 전자·조명·가전·사무기기 (과세) — 색상·소재명이 식품 키워드와 겹치는 오탐 방지
    # 예: '무드등 루미코 LED 충전식 (오렌지)' → '오렌지'(색상)가 면세 과일로 오탐
    # 예: '체리목 원목 책상' → '체리'(나무 종류)가 면세 과일로 오탐
    "LED", "조명", "무드등", "스탠드조명", "테이블조명", "플로어조명",
    "전구", "램프", "형광등", "할로겐", "매입등", "다운라이트",
    "충전식", "충전기", "배터리",
    "모니터", "디스플레이", "프로젝터", "스크린",
    "케이블", "어댑터", "허브", "스위치", "라우터",
    "키보드", "마우스", "헤드셋", "스피커", "웹캠",
    "노트북", "태블릿", "스마트폰", "핸드폰",
    "에어컨", "냉장고", "세탁기", "건조기", "청소기", "공기청정기",
    "선풍기", "서큘레이터", "제습기", "가습기",
    "프린터", "복합기", "스캐너", "팩스",
    "책상", "의자", "서랍", "캐비닛", "파티션", "칸막이",
    "커튼", "블라인드", "롤스크린",
    "카펫", "매트", "러그",
    "우산", "양산",
    "가방", "백팩", "캐리어",
    ...
    "신발", "운동화", "구두", "슬리퍼",
    "안전화", "안전모", "장갑", "보호대",
    # ── 유가증권·상품권류 (비과세) — 부가세법상 유가증권 자체가 과세 거래 대상 아님
    # 비과세가 정확한 분류이므로 상품명에 식품명이 포함돼도 면세 검토 불필요
    # 예: '기프티쇼 스타벅스 블루베리 케이크' → '블루베리' 탐지 차단 (비과세 정상)
    "기프티쇼", "기프트쇼",
    "상품권", "기프트카드", "gift card",
    "모바일상품권", "모바일 상품권",
    "이용권", "교환권", "쿠폰",
    ...
    "해피머니", "컬쳐랜드", "문화상품권",
    "도서상품권", "온라인상품권",
    # ── 독서·사무용 거치대류 (과세) — '책' '도서' 포함 상품명 오탐 방지
    # 예: '투명 아크릴 북레스트 스탠딩 독서대 거치대 책 진열대' → '책'이 면세 도서로 오탐
    "독서대", "북레스트", "북스탠드", "북홀더", "북스토퍼",
    "진열대", "거치대", "꽂이", "매거진랙", "매거진 랙",
]
...
_LARGE_CORP_RAW = [
    # ── 삼성그룹 (재계 1위) ─────────────────────────────────
    "삼성전자", "삼성전자판매", "삼성SDS", "삼성물산", "삼성SDI", "삼성디스플레이",
...
def _load_ftc_large_corps(path: "Path") -> set:
    ...
    def _names_from_df(df) -> set:
        ...
        # 열 탐지: "회사명" 헤더가 있는 열 우선 → 없으면 별첨5(5,7열) → 마지막 단순목록(0열)
        col_indices = None
        for r_idx in range(min(5, len(df))):
            row_vals = [str(df.iloc[r_idx, c]).strip() for c in range(ncols)]
            if "회사명" in row_vals:
                col_indices = [row_vals.index("회사명")]
                break
...
def _tax_kw_match(text: str, keywords: list):
    """상품명(name) 필드용 — 양쪽 경계 엄격 매칭.
    한글 키워드: 앞뒤 한글 차단.
    영문 포함 키워드: 앞뒤 영문자·숫자도 차단 (NOTEBOOK 내 ebook 오탐 방지).

    예) '보리' 검색: '아이보리' → 앞이 한글 '이' → ✗  /  '보리 선물세트' → ✓
    예) '밀' 검색 : '밀폐용기'  → 뒤가 한글 '폐' → ✗  /  '밀 1kg'       → ✓
    예) 'ebook' 검색: 'NOTEBOOK' → 앞이 영문 'B' → ✗  /  'ebook 구매'   → ✓
    """
    import re
    ...
    for kw in keywords:
        if re.search(r'[a-zA-Z]', kw):
            # 영문자 포함 키워드: 한글·영문자·숫자 모두 경계로 차단
            pattern = rf"(?<![가-힣a-zA-Z\d]){re.escape(kw)}(?![가-힣a-zA-Z\d])"
        else:
...
def _tax_kw_match_spec(text: str, keywords: list):
    """규격(spec) 필드용 — 오른쪽 한글 경계 + 왼쪽 숫자 경계 체크.
    복합어 식품명에서 성분 키워드 탐지:
      '밥다시마300g' → '다시마' 뒤가 숫자 → ✓  (왼쪽 한글은 허용)
      '쌀톳200g'     → '톳' 뒤가 숫자   → ✓
      '밀폐용기'     → '밀' 뒤가 '폐'(한글) → ✗ (오른쪽 차단)
      '전복구이'     → '전복' 뒤가 '구'(한글) → ✗ (오른쪽 차단)
      '3가지'        → '가지' 앞이 숫자 → ✗ (왼쪽 숫자 차단)
    영문 포함 키워드: 앞뒤 영문자·숫자도 차단.
    """
    import re
    if not isinstance(text, str):
        return None, False
    ...
    for kw in keywords:
        if re.search(r'[a-zA-Z]', kw):
            # 영문자 포함 키워드: 왼쪽 영문자·숫자 차단, 오른쪽 한글·영문자·숫자 차단
            pattern = rf"(?<![a-zA-Z\d]){re.escape(kw)}(?![가-힣a-zA-Z\d])"
        else:
            pattern = rf"(?<!\d){re.escape(kw)}(?![가-힣])"
        if re.search(pattern, t, re.IGNORECASE):
            return kw, True
    return None, False
...
def _read_excel_smart(path, search_cols=None, max_skip=10, **kwargs):
    ...
    파일 매직 바이트로 xlsx/xls 형식을 자동 감지하여 엔진 선택.
    search_cols 중 하나라도 있는 행을 헤더로 사용한다.
    """
    if search_cols is None:
        search_cols = ["주문번호", "구매문서번호", "일정산번호"]
    kwargs.pop("engine", None)   # 호출 측 engine 인자 무시 — 아래에서 자동 결정

    # 파일 시그니처로 엔진 결정:
    #   \xd0\xcf... → OLE2 복합문서 (.xls) → xlrd
    #   \x50\x4b... → ZIP (.xlsx/.xlsm)    → openpyxl
    _engine = "openpyxl"
    try:
        ...
        if magic[:2] == b'\xd0\xcf':
            try:
                ...
                raise RuntimeError(
                    "구형 .xls 파일이 감지되었으나 xlrd 패키지가 설치되어 있지 않습니다.\n"
                    "파일을 Excel에서 열고 [다른 이름으로 저장 → .xlsx] 로 변환 후 다시 시도하세요."
                )
        elif magic[:2] != b'\x50\x4b':
            # ZIP 시그니처도 OLE2도 아님 → 보안문서 또는 손상된 파일
            raise RuntimeError(
                f"파일을 읽을 수 없습니다: {Path(path).name}\n"
                "KT 보안문서(암호화)이거나 손상된 파일일 수 있습니다.\n"
                "Excel에서 직접 열어 일반 xlsx로 저장한 후 다시 시도하세요."
            )
    except RuntimeError:
        raise
    except Exception:
    ...
    for skip in range(max_skip):
        try:
            probe = pd.read_excel(path, header=skip, nrows=0, engine=_engine)
            if any(c in probe.columns for c in search_cols):
                return pd.read_excel(path, header=skip, engine=_engine, **kwargs)
        except Exception:
            pass
    return pd.read_excel(path, engine=_engine, **kwargs)
...
class MappingMaster:
    def __init__(self, path: Optional[Path], midcorp_path: Optional[Path] = None):
        ...
        self.midcorp_biz: Dict[str, str]             = {}    # 정규화 기업명 → 사업자번호
    ...
    def get_corp_biz_no(self, corp_name: str) -> str:
        """협력사명 → 사업자번호 반환 (중견기업만 자동, 나머지 빈문자열)."""
        norm = _normalize_corp_name(corp_name)
        return self.midcorp_biz.get(norm, "")
    ...
    def load(self, log):
        ...
        if self.midcorp_path and self.midcorp_path.exists():
            try:
                ...
                biz_col_m = next((c for c in df_mid.columns if "사업자" in c), None)
                if name_col_m:
                    for _, row in df_mid.iterrows():
                        n = str(row.get(name_col_m, "")).strip()
                        if not n:
                            continue
                        norm = _normalize_corp_name(n)
                        self.midcorp_set.add(norm)
                        if biz_col_m:
                            biz = str(row.get(biz_col_m, "")).strip()
                            if biz and biz != "nan":
                                self.midcorp_biz[norm] = biz
                    log.info(f"  중견기업목록 로드: {len(self.midcorp_set)}개 기업 "
                             f"(사업자번호 {len(self.midcorp_biz)}건 포함, 파일: {self.midcorp_path.name})")
                else:
        ...
        try:
            ...
            def _load_svccat(df_s, label):
                ...
                sub_c  = next((col for col in c if "정산용" in col and "중분류" in col), None)
                log.info(f"  [{label}] cat={cat_c}, acc={acc_c}, dept={dept_c}, mgr={mgr_c}, sub={sub_c}")
                result = {}
                for _, r in df_s.iterrows():
                    ...
                    if k_raw and k_raw.lower() not in ("nan", ""):
                        ...
                        result[k] = (
                            ...
                            str(r[sub_c]).strip()  if sub_c  else "",
                        )
                return result
    ...
    def apply(self, df: pd.DataFrame, merged_cols: Dict[str, Optional[str]], log) -> pd.DataFrame:
        ...
        def _svc_get(x: str, idx: int) -> str:
            ...
            return v[idx] if v and idx < len(v) else ""
        ...
        if svc_col and svc_col in df.columns:
            ...
            df["_sub_mapped"]  = df["_svc"].map(lambda x: _svc_get(x, 3))
...
class SettlementRunner:
    ...
    def __init__(self, base_dir=".", start_date=None, end_date=None, gui_mode=False):
        self.base       = Path(base_dir)
        self.today      = datetime.date.today()
        self.log        = self._setup_logger()
        self.gui_mode     = gui_mode
        self._override_ps = start_date   # GUI에서 주입된 시작일
        self._override_pe = end_date     # GUI에서 주입된 종료일
    ...
    def detect_files(self):
        ...
        self.log.info(f"  [탐색 기준폴더] {self.base.resolve()}")
        xlsx_files = [f for f in self.base.iterdir()
                      if f.suffix.lower() in (".xlsx", ".xlsm", ".xls")
                      and not f.name.startswith("~")]
        self.log.info(f"  [탐색된 xlsx]  {[f.name for f in xlsx_files] or '없음'}")
        ...
        def _find_kw(*keywords, exclude_kw=()):
            """폴더 내 xlsx 중 키워드를 모두 포함하는 파일 탐색 (대소문자 무시).
            exclude_kw 중 하나라도 포함되면 제외."""
            for f in xlsx_files:
                n = f.name.lower()
                if all(kw.lower() in n for kw in keywords):
                    if not any(ex.lower() in n for ex in exclude_kw):
                        return f
            return None

        # KT 정산 파일: 'kt' 포함 + 보안문서·암호화 파일 제외
        # ※ "보안문서": KT그룹사보안문서 암호화 파일
        # ※ "ktc": KTC 시스템 내부 암호화 포맷 (.xlsm이지만 실제 ZIP 구조 아님)
        _SEC = ("플랫폼", "담당자", "return", "mapping", "목록", "보안문서", "ktc")
        self.kt_path = (
            _find("kt_raw.xlsx", "KT_raw.xlsx", "kt_raw.xlsm")
            or _find_kw("kt", "정산", exclude_kw=_SEC)
            or _find_kw("kt", "자료", exclude_kw=_SEC)
            or _find_kw("kt", exclude_kw=(*_SEC, "과세", "중견", "대기업"))
        )
        # 플랫폼 파일: '통합플랫폼' or '플랫폼' or '일일정산관리' 포함
        self.pl_path = (
            _find("platform.xlsx", "플랫폼.xlsx", "통합플랫폼.xlsx")
            or _find_kw("통합플랫폼", exclude_kw=("보안문서",))
            or _find_kw("일일정산관리", exclude_kw=("보안문서", "ktc"))
            or _find_kw("플랫폼", exclude_kw=("mapping", "return", "kt", "보안문서"))
        )
        self.rm_path = _find("return_mapping.xlsx")
        # mapping_master: '관리회계' or '서비스카테고리' or '서비스 카테고리' 포함
        mm_path = (
            _find("mapping_master.xlsx", "매핑마스터.xlsx")
            or _find_kw("관리회계", exclude_kw=("return", "mapping"))
            or _find_kw("서비스", "카테고리", exclude_kw=("return",))
        )
        ...
        for label, path, req in [
            ...
            if req and not path:
                if self.gui_mode:
                    self.log.error(f"  필수 파일 '{label.strip()}' 을(를) 찾을 수 없습니다. 폴더를 확인하세요.")
                    raise FileNotFoundError(f"필수 파일 미탐지: {label.strip()}")
    ...
    # ── 정산 기간 ──────────────────────────────────────────────
    def get_period(self):
        # GUI 또는 외부에서 날짜가 주입된 경우 우선 사용
        if self._override_ps and self._override_pe:
            self.log.info(f"  정산 기간: {self._override_ps} ~ {self._override_pe} (수동 지정)")
            return self._override_ps, self._override_pe
    ...
    def step0b_validate_tax(self):
        ...
        # 원래 과세구분 값 보존 (정규화 전 — 출력 표시용)
        # ※ '영세'→'면세' 정규화로 현재과세구분 열이 왜곡되지 않도록 분리 관리
        df_p['_tax_orig'] = df_p[tax_c].astype(str).str.strip()
        # 검증 비교 로직용 정규화 (영세/영세율 → 면세)
        ...
        for _, r in df_p.iterrows():
            ...
            cur_tax      = _s(r.get(tax_c,        ""))   # 정규화된 값 (비교 로직용)
            cur_tax_disp = _s(r.get('_tax_orig',  "")) or cur_tax  # 원래 값 (출력용)
            combined     = f"{name} {spec}"
            row_code     = _re_corp.sub(r'\.0$', '', _s(r.get(code_c, ""))) if code_c else ""
            ...
            if known_kw and cur_tax != "면세":
                _tax_note = ("비과세로 등록되어 있으나" if cur_tax_disp == "비과세"
                             else f"{cur_tax_disp}로 등록되어 있으나")
                self.tax_issues.append({
                    "주문번호"    : _s(r.get(ord_c,  "")) if ord_c  else "",
                    "상품코드"    : _s(r.get(code_c, "")) if code_c else "",
                    "상품명"      : name,
    ...
    def step1_filter(self):
        ...
        # KT↔플랫폼 연동 딜레이로 승인일이 정산 마감일 다음 날(pe+1)에 찍히는 경우가 있어
        # 플랫폼 필터를 pe+1일까지 허용 (1일 여유)
        _pe_ext = self.pe + datetime.timedelta(days=1)
        if dc_pl_apv and dc_pl_apv in df_pl.columns:
            ...
            self.df_pl = df_pl[(df_pl[dc_pl_apv] >= self.ps) &
                               (df_pl[dc_pl_apv] <= _pe_ext)].copy().reset_index(drop=True)
            self.log.info(f"  플랫폼 승인일(입고승인일) 기준 필터: {dc_pl_apv} ({self.ps}~{_pe_ext}, +1일 여유)")
            ...
            self.df_pl = df_pl[(df_pl[dc_pl_settle] >= self.ps) &
                               (df_pl[dc_pl_settle] <= _pe_ext)].copy().reset_index(drop=True)
            ...
            self.df_pl = df_pl[(df_pl[dc_pl_in] >= self.ps) &
                               (df_pl[dc_pl_in] <= _pe_ext)].copy().reset_index(drop=True)
    ...
    def _auto_match_return(self, ret_row: pd.Series, extra_exclude: set | None = None):
        ...
        # 이미 일반 주문 또는 앞선 반품 자동매칭에서 사용된 플랫폼 행 제외
    ...
    def _route_b(self):
        ...
        ok_list, ng_rows = [], []
        _auto_used_keys: set = set()   # 앞선 반품에서 이미 사용한 플랫폼 키 (중복 매칭 방지)
        for _, r in self.df_ret_kt.iterrows():
            pl_key, pl_amt = self._auto_match_return(r, extra_exclude=_auto_used_keys)
            if pl_key:
                _auto_used_keys.add(pl_key)
        ...
        # ── 공통: 플랫폼 반품 후보 탐색 헬퍼 ───────────────────────────
        _pl_for_suggest = getattr(self, "df_pl_full", self.df_pl)
        pl_amt_col_s  = self.pl_cols["정산금액"]
        pl_corp_col_s = self.pl_cols.get("협력사명")
        pl_ord_col_s  = self.pl_cols.get("주문번호") or self.pl_cols["주문번호"]
        pl_item_col_s = self.pl_cols.get("품목번호")

        # 플랫폼에서 반품(음수 금액)인 행만 후보로 사용
        _pl_ret_mask = pd.to_numeric(
            _pl_for_suggest[pl_amt_col_s], errors="coerce"
        ) < 0
        _pl_ret_pool = _pl_for_suggest[_pl_ret_mask].copy() if _pl_ret_mask.any() else _pl_for_suggest.copy()

        # 이미 자동매칭된 키 제외
        _used_suggest: set = set()
        if self.df_matched is not None and "_match_key" in self.df_matched.columns:
            _used_suggest |= set(self.df_matched["_match_key"].astype(str))
        for item in ok_list:
            if "_ret_pl_key" in item:
                _used_suggest.add(str(item["_ret_pl_key"]))

        def _find_pl_candidates(r) -> list[dict]:
            """KT 반품 행에 대해 플랫폼 후보를 반환 (금액·협력사 기준, 최대 3건)."""
            kt_amt  = pd.to_numeric(r.get(amt_col), errors="coerce")
            kt_corp = str(r.get(corp_col, "")).strip() if corp_col else ""
            if pd.isna(kt_amt):
                return []
            pool = _pl_ret_pool[~_pl_ret_pool["_match_key"].isin(_used_suggest)].copy()
            # ① 금액 ±1원 필터
            pl_amts = pd.to_numeric(pool[pl_amt_col_s], errors="coerce")
            pool = pool[(pl_amts.abs() - abs(kt_amt)).abs() <= 1]
            if pool.empty:
                return []
            # ② 협력사명 필터 (맞는 것만 남김, 없으면 금액만으로)
            if kt_corp and pl_corp_col_s and pl_corp_col_s in pool.columns:
                corp_m = pool[pl_corp_col_s].astype(str).str.strip() == kt_corp
                if corp_m.any():
                    pool = pool[corp_m]
            candidates = []
            for _, pr in pool.head(3).iterrows():
                pl_ord  = str(pr.get(pl_ord_col_s, "")).strip()
                pl_item = ""
                if pl_item_col_s and pl_item_col_s in pr.index:
                    _raw = str(pr.get(pl_item_col_s, "")).strip()
                    try:
                        pl_item = str(int(float(_raw))) if _raw not in ("", "nan") else ""
                    except (ValueError, TypeError):
                        pl_item = _raw
                candidates.append({"pl_ord": pl_ord, "pl_item": pl_item,
                                    "pl_key": str(pr.get("_match_key", ""))})
            return candidates
        ...
        def _make_rm_template(target_df, reason_label):
            ...
            rows = []
            used_in_template: set = set()
            auto_count = 0
            for _, r in target_df.iterrows():
                cands = _find_pl_candidates(r)
                # 이미 같은 후보를 앞 행이 사용했으면 제거 (동일 금액 중복 방지)
                cands = [c for c in cands if c["pl_key"] not in used_in_template]

                if cands:
                    best = cands[0]
                    used_in_template.add(best["pl_key"])
                    pl_ord_val  = best["pl_ord"]
                    pl_item_val = best["pl_item"]
                    confidence  = "자동(금액+협력사 일치)"
                    auto_count += 1
                else:
                    pl_ord_val  = ""
                    pl_item_val = ""
                    confidence  = "후보없음 — 직접 입력 필요"

                rows.append({
                    "★플랫폼반품주문번호": pl_ord_val,
                    "★플랫폼품목번호"    : pl_item_val,
                    "후보신뢰도"         : confidence,
            ...
            n        = len(rows)
            need_manual = n - auto_count
            auto_msg = (f"  (자동매칭 성공 {len(ok_list)}건은 별도 처리됩니다)\n"
                        if ok_list else "")
            if auto_count == n:
                action_msg = (
                    f"  ★ 전건 자동 제안 완료 — 내용 확인 후 저장하고 재실행하세요.\n"
                    f"  후보신뢰도 열이 '후보없음'인 행만 A·B열을 직접 입력하세요.\n"
                )
            elif auto_count > 0:
                action_msg = (
                    f"  ★ {auto_count}건 자동 제안 / {need_manual}건 직접 입력 필요\n"
                    f"  '후보없음' 행의 A열(★플랫폼반품주문번호)·B열(★플랫폼품목번호)만 입력 후 재실행\n"
                )
            else:
                action_msg = (
                    f"  ★ 작성 방법:\n"
                    f"  A열(★플랫폼반품주문번호): 통합플랫폼에서 대응 반품의 주문번호 입력\n"
                    f"  B열(★플랫폼품목번호): 해당 품목번호 입력\n"
                    f"  작성 후 저장하고 재실행하세요.\n"
                )
            self.log.info(
                f"\n{'='*60}\n"
            ...
            raise ReturnMappingNeeded(
                f"반품 매핑 파일 생성 완료 ({n}건, 자동제안 {auto_count}건) → {rm_out}\n"
                f"내용 확인 후 저장하고 재실행해주세요."
            )
    ...
    def build_invoice_df(self) -> pd.DataFrame:
        ...
        # 플랫폼누락 행: 플랫폼 주문번호 컬럼에 KT 구매문서번호 채움 (식별 목적)
        # ※ 플랫폼 컬럼 룩업(위) 이후에 실행해야 NaN 덮어쓰기가 방지됨
        _missing_mask = df["대사결과"] == "플랫폼누락"
        if _missing_mask.any():
            _pl_ord_c = self.pl_cols.get("주문번호")
            _kt_ord_c = self.kt_cols.get("주문번호")   # "구매문서번호"
            if _pl_ord_c and _kt_ord_c and _kt_ord_c in df.columns:
                if _pl_ord_c not in df.columns:
                    df[_pl_ord_c] = None
                df.loc[_missing_mask, _pl_ord_c] = (
                    df.loc[_missing_mask, _kt_ord_c].astype(str)
                    .str.replace(r'\.0$', '', regex=True).str.strip()
                )
                self.log.info(
                    f"  플랫폼누락 {int(_missing_mask.sum())}건: "
                    f"주문번호 컬럼 ← KT {_kt_ord_c} 값으로 채움"
                )
    ...
    def write_invoice_sheet(self, wb):
        ...
        # ── 주문&품목 복합키 컬럼 생성 (주문번호&품목번호) ──────────
        _ord_c  = self.pl_cols.get("주문번호")
        _item_c = self.pl_cols.get("품목번호")
        if _ord_c and _item_c and _ord_c in df.columns and _item_c in df.columns:
            def _strip_dot0(v):
                s = str(v) if not pd.isna(v) else ""
                return s[:-2] if s.endswith(".0") else s
            df["주문&품목"] = (df[_ord_c].map(_strip_dot0)
                               + df[_item_c].map(_strip_dot0))

        # ── 컬럼 순서 정렬 (사용자 지정 목차 기준) ──────────────────
        # ※ 이 목록에 없는 컬럼은 출력하지 않음 (KT 원본 컬럼 등 자동 제외)
        # ※ "대사결과"는 반드시 목록에 포함 — 행 색상·정렬 로직이 이 컬럼을 참조함
        _DESIRED_ORDER = [
            "대사결과",
            "일정산번호", "정산확정일", "일일정산월",
            "주문번호", "품목번호", "주문&품목",
        ...
        # 목록에 있는 컬럼만 순서대로 출력 (없는 컬럼은 자동 스킵)
        _seen = set()
        _ordered = []
        ...
        ws.freeze_panes = "A5"
        ws.auto_filter.ref = ws.cell(4, 1).coordinate + ":" + ws.cell(4, len(cols)).coordinate
        auto_col_width(ws)
    ...
    def write_bill_sheet(self, wb):
        ...
        if self.master and _mc_cnt > 0 and self.df_invoice is not None:
            ...
            _mc_hits = [_cn for _cn in _corps
                        if _normalize_corp_name(str(_cn)) in self.master.midcorp_set]
            self.log.info(
                f"    중견기업 매칭: 전체 협력사 {len(_corps)}개 중 {len(_mc_hits)}개 → {_mc_hits}"
            )
        ...
        grp["어음대상"] = grp.apply(
            lambda r: "발행대상" if (r.cond1 and r.cond2 and r.cond3)
                 else ("확인필요" if (r.cond1 and r.cond2 and not r.cond3)
                 else "미해당"),
            axis=1
        )
        _PAY_DAYS = {"대기업": 90, "중견기업": 60}

        def _bill_bigo(r):
            if r["어음대상"] == "발행대상":
                biz_no  = self.master.get_corp_biz_no(r["협력사명"]) if self.master else ""
                days    = _PAY_DAYS.get(r["기업규모"], "")
                amt_str = f"{int(r['VAT포함합계']):,}원" if pd.notna(r["VAT포함합계"]) else ""
                day_str = f" {days}일 지급대상" if days else ""
                return f"{amt_str}_{r['기업규모']}{day_str}_{r['협력사명']}_{biz_no}"
            if r["대사결과"] != "정상":
                return "대사 미확인"
            if not r.cond1:
                return "매입금액미달(VAT포함)"
        ...
        grp["비고"] = grp.apply(_bill_bigo, axis=1)
        ...
        set_col_widths = [22, 20, 18, 18, 12, 12, 50]
    ...
    def run(self):
        try:
            ...
            # 파일명: {연도}년 {월}월 {1|2}차 정산_KTC_{오늘날짜}
            _ps_year  = self.ps.year
            _ps_month = self.ps.month
            _round    = (getattr(self, "_round_override", None)
                         or ("1차" if self.ps.day <= 15 else "2차"))
            _base_name = f"{_ps_year}년 {_ps_month}월 {_round} 정산_KTC_{today_str}"
            result_path = str(self.base / f"{_base_name}.xlsx")
            ...
            for _suffix in [""] + [f"_{i}" for i in range(1, 10)]:
                _try_path = str(self.base / f"{_base_name}{_suffix}.xlsx")
                try:
                    wb.save(_try_path)
                    result_path = _try_path
                    self.result_path = _try_path   # GUI에서 참조용
                    _saved = True
                    break
            ...
            if not _saved:
                raise PermissionError(
                    ...
                    f"Excel에서 '{_base_name}*.xlsx' 파일을 모두 닫은 후 재실행하세요."
                )
...
if __name__ == "__main__":
    if getattr(sys, "frozen", False):
        # PyInstaller exe: exe 파일이 있는 폴더를 작업 기준으로 사용
        _base = str(Path(sys.executable).parent)
    else:
        _base = "."
    SettlementRunner(base_dir=_base).run()
