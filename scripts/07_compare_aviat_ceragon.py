...
def main():
    ...
    asks = [
        ...
        "섞인 것인지 불명확 - 확인 필요. 추가로: 위 6개 품목이 발주이력상 각각 2개씩 관측됐는데, "
        "이 '2'가 (a)링크 양단 사이트 합산 총수량인지 (b)사이트 1곳당 수량(따라서 링크 전체는 "
        "4개)인지 자체가 확인되지 않았다 - 14번 파일에서는 Ceragon 자료의 A/B국소 합산 관례를 "
        "적용해 (a)로 해석했으나(19,088,472원의 근거), 이는 정황상 가정일 뿐 Aviat 공식 확인이 "
        "아니므로 반드시 확인 요청.",
