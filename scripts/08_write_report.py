...
def main():
    ...
    idx7t, rows7t = ws_data("07_Aviat_Ceragon_구성가격비교.xlsx", "구성총액_비교")
    aviat_11ghz20_total = next(
        r[idx7t["Aviat 판매총액(추정)"]] for r in rows7t
        if r[idx7t["구성방식"]] == "2+0" and r[idx7t["Ceragon 대역"]] == "11GHz"
        and r[idx7t["Aviat 판매총액(추정)"]] is not None
    )
    ...
    lines.append(
        ...
        f"- 데이터 품질 이슈(확인 필요): 6GHz용 'Configuration SW Package' 8개 품목의 설명 텍스트에 "
        f"적힌 'Nnd-Activation' 표기가 2+0 SD=2nd부터 8+0 Non_SD=9nd까지 순차 증가하는데, 같은 "
        f"계열인 8GHz/11GHz용 8개 품목은 전부 '2nd-Activation'으로 동일하다 - 원본 가격표 작성 시 "
        f"행 복사 과정에서 번호가 밀린 것으로 추정되며(6GHz만 8개 값이 다르게 나올 기술적 이유가 "
        f"없음), Ceragon 최종 단가표 확정 전 정정 확인이 필요하다(07번 파일 공급사확인사항 시트 "
        f"참조).\n"
    ...
    lines.append(
        "**Aviat와 Ceragon은 8GHz 및 11GHz 대역이 공통으로 존재한다. 다만 Aviat의 구성방식별 완성 "
        "BoM이 아직 공식적으로 확인되지 않아 구성 총액 비교는 보류한다.**\n\n"
        "2+0/4+0/6+0/8+0 × 8GHz/11GHz × SD/Non_SD 총 16개 조합을 검토한 세부 내용은 다음과 같다"
        "(돈현님 피드백, 2026-09-04: '대역이 겹치지 않는다'고 서술하지 않는다).\n\n"
        f"- 2+0/4+0(8GHz·11GHz, 8건): **보류** — 8장(04v2)에서 확인한 대로 Aviat 측 후보 근거가 "
        f"유력/참고 후보 수준까지는 있으나 완성된 표준 BoM으로 확정되지 않아, 구성 총액 단위의 "
        f"전면 비교를 보류한다. 그중 11GHz 2+0은 유력 후보 6개 품목의 실제 링크 전체 수량(양쪽 "
        f"사이트 합산, 품목당 2개씩) 기준 참고 판매총액을 {aviat_11ghz20_total:,.0f}원으로 산출할 "
        f"수 있었으나, 이는 Ceragon의 전체 품목(18~27종) 총액과 항목 수 자체가 달라 직접 비교 "
        f"가능한 값이 아니다(2026-09-08 정정: 이전 버전은 품목당 배수를 그대로 단가에 곱해 실제 "
        f"총액의 절반으로 축소 계산되어 있었음).\n"
    ...
    lines.append(
        ...
        "12. Ceragon 6GHz 'Configuration SW Package' 8개 품목의 'Nnd-Activation' 표기가 2nd~9nd로 "
        "순차 증가하는 것(8/11GHz용 동일 계열은 전부 2nd) - 원본 가격표 행 복사 오류로 추정, 6GHz "
        "쪽 전체를 2nd로 통일해야 하는지 확인 필요(11장 참조).\n"
    )
