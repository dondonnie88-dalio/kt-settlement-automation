...
def main():
    ...
    lines.append(
        ...
        "- 8GHz 2+0/4+0과 11GHz 4+0은 참고 후보 수준에 그친다 — 다중 대역이 섞인 대형 이벤트 비중이 "
        "높아 반복 관측으로 뒷받침되는 품목이 아직 없다(공급사 확인 필요).\n"
    ...
    lines.append("\n## 12. 가격차가 큰 구성\n")
    lines.append(
        "구성 총액 단위 비교는 11장과 같이 보류/불가 상태이므로, 개별 품목 직접대응 비교(3건) "
        "결과만 보고한다:\n\n"
    ...
    lines.append(
        ...
        "3. 8GHz/11GHz 6+0/8+0(IAP3 계열) 구성 존재 여부 자체 확인, 4GHz 6+0 구성 존재 여부 "
        "확인(Aviat).\n"
        "4. Aviat 8GHz/11GHz 2+0/4+0 구성에 대해 04v2(8장)에서 식별한 유력/참고 후보 품목·수량이 "
        "실제 표준 BoM과 일치하는지 확인 회신(특히 11GHz 2+0 유력 후보 6개 품목), Ceragon 4GHz "
        "대역 지원 여부.\n"
