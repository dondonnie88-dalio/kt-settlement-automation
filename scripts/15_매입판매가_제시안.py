...
# '고객 제시가(최초 견적)'를 만든다. 이 상수만 바꾸면 전체 시트에 일괄 반영된다.
# 2026-09-14 조정(8%→3%p): 고객사가 협상 없이 최초 제시가를 그대로 수락할 가능성이 있어,
# 표준 마진(2~5%) 위에 버퍼까지 그대로 이익으로 굳어지면 폭리로 비칠 수 있다는 지적 반영 -
# 그대로 수락돼도 '표준 마진 범위 내 약간의 여유'로 보일 만큼만 남긴다.
ANCHOR_BUFFER = 0.03
...
def build_guide_sheet(wb, n_total, n_exact):
    ...
    guide = [
        ...
        ("고객 제시가(버퍼) 열(2026-09-11 추가, 2026-09-14 조정)", f"돈현님 요청: 고객사와의 "
                              ...
                              f"마지노선이고, 여기에 {ANCHOR_BUFFER*100:.0f}%p를 얹은 '고객 "
                              f"제시가(최초견적)'를 추가했다 - 고객사가 인하를 요청하면 목표가 "
                              f"선까지 내려가며 마무리하는 구조다. 처음엔 8%p로 시작했으나, 고객사가"
                              f" 협상 없이 최초 제시가를 그대로 수락할 가능성도 있고, 그 경우 표준"
                              f" 마진(2~5%) 위에 버퍼까지 그대로 이익으로 굳어져 폭리로 비칠 수"
