...
def load_band_rows(sheet_name):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        rows.append({
            ...
            "판정수준": row[idx["판정수준"]],
            # 애니콤 회신은 링크 전체(2+0/4+0 등 구성 하나 전체, 양쪽 사이트 합산) 기준 수량으로
            # 올 것이므로, N당 배수가 아니라 실제 링크 전체 수량 열을 대조 기준으로 쓴다
            # (2026-09-08 정정 - 이전 버전은 배수를 그대로 써서 2+0/4+0 실제 수량의 1/2~1/4이었음).
            "대표수량": row[idx["추정 수량(N+0 링크 전체 가정, 양쪽 사이트 합산)"]],
...
def main():
    ...
    guide = [
        ...
        ("대표수량 '변동, 공급사 확인 필요'", "발주이력상 관측 비율이 30% 이상 벌어져 대표 수량을 "
                                        "임의로 정하지 않은 품목이다. 이런 행은 '내부 대표수량' 셀이 "
                                        "연한 빨간색으로 표시되어 있다 - 회신 입력 전부터 눈에 띄도록 "
                                        "고정 표시한 것이며, 04v2 파일과 동일한 색상 규칙이다. 이런 "
                                        "행은 수량 차이를 계산하지 않고 '내부 추정 변동(비교 보류)'로 "
                                        "표시한다 - 애니콤 회신 수량을 그대로 신뢰하고 참고용으로만 "
                                        "기록한다(내부 추정과 다르다고 '차이 있음'으로 오판하지 말 것)."),
    ...
    for sheet_name in BAND_SHEETS:
        ...
        for row in rows:
            ...
            # 04v2와 동일한 색상 규칙: 관측 비율이 30% 이상 벌어져 대표 수량을 매기지 않은
            # 품목은 회신 입력 전에도(자동판정 문구가 나오기 전에도) 눈에 띄도록 대표수량
            # 셀 자체를 표시해둔다 - 이 행의 '차이 있음' 결과는 비교 기준으로 쓰지 않는다.
            if rep == "변동, 공급사 확인 필요":
