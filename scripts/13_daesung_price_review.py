...
def main():
    ...
    ws1.append(["시한 내 권고", "2026-09-16까지 애니콤 공식 회신이 오면 그 수치로 재검토. 회신이 "
                              ...
                              "근거도 아님 - 별개의 문제)."])
    review_note_row = ws1.max_row + 1
    ws1.append(["검토의견(안) - 2026-09-09 합의",
                "2+0/4+0(8GHz·11GHz): 대성인포텍이 제출한 Ceragon 제안가격에 대해 Aviat 측 "
                "표준 BoM 또는 동일 범위 가격자료가 확보되지 않아 직접적인 가격 우위·열위 "
                "판단은 불가능함. 다만 현재 확보된 자료 범위 내에서는 대성 제안가격이 현저히 "
                "과다하다는 객관적 근거 역시 확인되지 않음. 따라서 본 건은 가격 적정성이 "
                "입증되었다고 보기는 어려우나, 반대로 부적정하다고 판단할 근거도 부족하므로 "
                "현 시점에서는 조건부 수용 가능으로 판단함(요컨대 '싸다/비싸다'가 아니라 "
                "'가격 인하 요구의 근거가 부족하다'는 뜻). 6+0/8+0: Aviat 발주이력에 이 구성을 "
                "시사하는 근거 자체가 전혀 없어 위 2+0/4+0과는 근거 성격이 다르다 - '반박 근거 "
                "없음'이 아니라 '비교 대상 자체가 없음'이며, 별도의 시장가 조사 없이는 이 두 "
                "구성만 따로 조건부 수용 여부를 판단하기 어려움. 향후 애니콤 회신 또는 추가 "
                "시장자료 확보 시 전체 재검토를 권고함."])
    finalize_sheet(ws1, 1, 2, ws1.max_row)
    ws1.column_dimensions["B"].width = 100
    for r in range(2, ws1.max_row + 1):
        mark_fill(ws1, r, 1, REVIEW_FILL)
    mark_fill(ws1, review_note_row, 1, CONFIRMED_FILL)
