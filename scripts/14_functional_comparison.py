...
def load_aviat_band(band, cfg):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        rows.append({
            ...
            "수량": row[idx["추정 수량(N+0 링크 전체 가정, 양쪽 사이트 합산)"]],
...
def build_guide_sheet(wb):
    ...
    guide = [
        ...
        ("중요한 원칙", "이 대응은 'K코드 A와 K코드 B가 같은 품목'이라는 뜻이 아니다 - 서로 "
                     ...
                     "단계로 취급한다 - '확정'이 아니다."),
        ("가장 중요한 한 문장(2026-09-08, 3차 검토에서 합의)",
                     "11GHz 2+0 유력후보 19,088,472원(링크 환산 가정, 내부 참고값)이 Ceragon "
                     "핵심11종 합계(35~47백만원)보다 작게 나오는 것은 'Aviat가 더 싸다'는 뜻이 "
                     "아니라 'Aviat 유력후보가 아직 완성 BoM 금액이 아니다'라는 뜻이다 - 11개 "
                     "핵심 기능군 중 본체/무선송수신부/팬/라이선스/마운트 5개만 채워져 있고, "
                     "대역결합기·인터페이스카드·제어카드·전원·SFP는 유력후보 근거가 없다. 지금 "
                     "단계에서 방어 가능한 실무 결론은 '대성 가격이 비싸다/싸다'가 아니라 "
                     "'같은 범위로 비교할 근거가 아직 부족하다'이다."),
        ...
        ("핵심 구조적 발견 (2) 대역결합기 등장 시점 - 확인된 사실 vs 추정 구분", "확인된 사실: "
                                             ...
                                             "단계로 취급한다."),
        ("수량 환산 가정(왜 관측수량을 링크 전체 수량으로 해석하는가 - 확정 아님)",
                                             "확인되는 사실은 하나뿐이다: 11GHz 2+0에서 Chassis/"
                                             "License/Fan/Mount 4개 품목 모두 Aviat 발주이력상 "
                                             "원 관측수량이 2였고, Ceragon이 공식 제출한 같은 "
                                             "기능군의 링크 총수량도 4개 품목 모두 2였다(수치 "
                                             "일치). 이 표는 Ceragon의 명시적 관례(A국소+B국소="
                                             "총수량, 두 끝 사이트 합산이 링크 총량)를 Aviat "
                                             "원 관측수량에도 적용해 '링크 전체 수량'으로 해석"
                                             "한다(배수×N=원 관측수량이 되도록 정의). 그러나 이 "
                                             "수치 일치는 해석을 뒷받침하는 정황 증거일 뿐, Aviat "
                                             "수량 정의 자체를 증명하지 않는다 - Aviat의 '2'가 "
                                             "우연히 같았을 수도 있고, 한 발주에 링크 2개분이 "
                                             "포함됐을 가능성도 원칙적으로 배제되지 않는다. 따라서"
                                             " 19,088,472원은 '발주이력에서 핵심 품목이 각각 2개로"
                                             " 관측됐고, 이를 링크 전체 수량으로 해석하면 나오는 "
                                             "값'이며, Ceragon 수량과 정합하지만 Aviat 공식 BoM "
                                             "확인 전까지는 확정이 아니라 '링크 환산 가정'으로 "
                                             "표시한다(07번 파일 공급사확인사항에 이 가정 자체를 "
                                             "확인 질문으로 포함)."),
        ...
        ("가장 근거가 뚜렷한 세트 - 그러나 아직 직접 비교는 불가", "11GHz 2+0만 유력 후보"
                                ...
                                "팬/라이선스/마운트 5개 카테고리, 6개 품목, 링크 환산 가정 기준"
                                " 참고 판매총액 19,088,472원(위 '수량 환산 가정' 항목 참조 - "
                                "확정 아님). 그러나 이 5개 카테고리는 11개 핵심"
...
def build_summary_sheet(wb, results):
    ...
    headers = ["구성", "Aviat 유력후보 카테고리 수(11개 중)",
               "Aviat 유력후보 합계(해당 카테고리만, 링크 환산 가정)",
               "Aviat 참고후보 합계(안정, 핵심11종, 링크 환산 가정)",
